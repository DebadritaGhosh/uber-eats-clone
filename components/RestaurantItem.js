    ...
    return (
        <>
            {restaurantData.map((res, i) => (
                <TouchableOpacity key={i} activeOpacity={1} style={{ marginBottom: 30 }}>
                    <View style={
                        {
                            marginTop: 10, padding: 15, backgroundColor: "white"
                        }
                    }>
                        <RestaurantImage img={res.image_url} />
                        <RestaurantInfo name={res.name} categories={res.categories} price={res.price} reviews={res.reviews} rating={res.rating} />
                    </View>
                </TouchableOpacity>
            ))}
        </>
